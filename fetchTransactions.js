...
const fetchTransactions = async (address, timestampAfter, timeStampBefore) => {
  ...
  for (let i = 0; i < confirmedSignatures.length; i++) {
    ...
    if (
      timeStampBefore >= transactionTime &&
      transactionTime >= timestampAfter
    ) {
      ...
      for (const instruction of transactionMessage.instructions
        ...
        if (
          ...
          instruction.parsed.info.destination ===
            'stakepotaccount57BfdN1ERzvLAgkXmuJQJcN9vsPa'
        ) {
...
async function transferSol() {
  const connection = new Connection('http://127.0.0.1:8899/', 'confirmed');
  const wallet = fs.readFileSync('/Users/raj/.config/koii/id.json');
  ...
  const transaction = new Transaction()
    .add(
      SystemProgram.transfer({
        ...
        toPubkey: 'stakepotaccount57BfdN1ERzvLAgkXmuJQJcN9vsPa',
